...
function updateWardensDebtMapTile(tileId, updater) {
  return updateWardensDebtGameState(gameState => {
    gameState.board.mapTiles = (gameState.board.mapTiles || []).map(tile =>
      tile.id === tileId ? updater({ ...tile }) : tile
    );
    return gameState;
  });
}
...
function deleteWardensDebtMapTile(tileId) {
  return updateWardensDebtGameState(gameState => {
    gameState.board.mapTiles = (gameState.board.mapTiles || []).filter(tile => tile.id !== tileId);
    gameState.board.mapTileIds = (gameState.board.mapTileIds || []).filter(id => id !== tileId);
    return gameState;
  });
}
...
function updateWardensDebtFigurePosition(figureId, updater) {
  return updateWardensDebtGameState(gameState => {
    const current = gameState.board.figurePositions?.[figureId] || { x: 0, y: 0 };
    gameState.board.figurePositions = {
      ...(gameState.board.figurePositions || {}),
      [figureId]: updater({ ...current }),
    };
    return gameState;
  });
}
...
function deleteWardensDebtFigure(figureId) {
  return updateWardensDebtGameState(gameState => {
    gameState.convicts = (gameState.convicts || []).filter(c => c.id !== figureId);
    gameState.enemies = (gameState.enemies || []).filter(e => e.instanceId !== figureId);
    if (gameState.board?.figurePositions) {
      const { [figureId]: _removed, ...rest } = gameState.board.figurePositions;
      gameState.board.figurePositions = rest;
    }
    if (gameState.zones?.board) {
      gameState.zones.board = (gameState.zones.board || []).filter(id => id !== figureId);
    }
    return gameState;
  });
}
...
function addWardensDebtPlaceholderFigure(kind, defId) {
  ...
  return updateWardensDebtGameState(gameState => {
    ...
    if (kind === 'convict') {
      ...
      gameState.convicts = [...gameState.convicts, {
        id,
      ...
      gameState.board.figurePositions[id] = placement;
      return gameState;
    }
    ...
    if (kind === 'enemy') {
      ...
      gameState.zones = { ...gameState.zones, board: [...(gameState.zones.board || []), instanceId] };
      gameState.board.figurePositions[instanceId] = placement;
      return gameState;
    }
...
function addWardensDebtPlaceholderMapTile(tileId) {
  ...
  return updateWardensDebtGameState(gameState => {
    ...
    if (!gameState.board.figurePositions) gameState.board.figurePositions = {};
    return gameState;
  });
}
...
function adjustSelectedHp(delta) {
  ...
  const ctx = wardensDebtSelectionContext(sel);
  if (!ctx) return false;
  const currentHp = sel.kind === 'wd-convict'
...
function wardensDebtObjectPanel(kind, idx) {
  ...
  const position = wardensDebtFigurePosition(runtime, isConvict ? obj.id : obj.instanceId);
  ...
  return `
    ...
    <div class="sp-obj-header sp-obj-header--selected${isConvict ? '' : ' sp-obj-header--monster'}">
      <div class="sp-obj-info">
        ...
        <div class="sp-meta-line">${escHtml(isConvict ? obj.convictDefId : obj.monsterCardId)}${angle ? ` · ${angle}°` : ''}</div>
      </div>
    </div>
...
function wardensDebtMapTilePanel(runtime, tileId) {
...
function wdSearchBody(query, runtime, placement) {
  ...
  for (const def of runtime.index.convictDefsById.values()) {
    if (def.name.toLowerCase().includes(q))
      results.push(`<button class="ap-item-btn ap-item-btn--monster" data-action="wd-add-figure" data-figure-kind="convict" data-figure-def-id="${escHtml(def.id)}" ${dis}>${escHtml(def.name)}</button>`);
  }
  for (const card of runtime.index.monsterCardsById.values()) {
    if (card.name.toLowerCase().includes(q))
      results.push(`<button class="ap-item-btn ap-item-btn--monster" data-action="wd-add-figure" data-figure-kind="enemy" data-figure-def-id="${escHtml(card.id)}" ${dis}>${escHtml(card.name)}</button>`);
  }
...
function addPanel() {
  ...
  const placement = cellSelection
    ...
    : null;
  const cellLabel = placement ? `${placement.x}, ${placement.y}` : 'Select a cell to place pieces';
