...
- Current implementation folder: `WardensDebt\`
...
- `WardensDebt/scripts/wardensDebt/schema.js`
- `WardensDebt/data/wardens-debt/core-set.json`
...
- a shared runtime store used by both the debug panel and the reused WardensDebt UI shell
...
The current app under `WardensDebt\` is a vanilla web app:
...
- `WardensDebt/index.html` defines a large SVG board, a right sidebar, a bottom action bar, and a mobile shell.
- `WardensDebt/scripts/main.js` wires together render, controls, drag, sidebar, elements, level, share, and mobile modules.
- `WardensDebt/scripts/state.js` is the central state store and persists runtime data in the URL hash using Base64-encoded JSON.
- `WardensDebt/scripts/sidebar.js` contains a large amount of reusable inspector/editor interaction logic:
...
- `WardensDebt/scripts/mobile.js` already provides a separate mobile interaction shell with panel switching and quick actions.
- `WardensDebt/scripts/data.js` and `scripts/games/*.js` use table-driven game data rather than hard-coding everything directly in render logic.
- `WardensDebt/scripts/wardensDebt/content.js` loads a Wardens Debt content pack and validates it through the new schema module.
...
- Do not force Wardens Debt map tiles into the legacy hex grid.
...
The launch behavior is inherited from the original HavenMap framework. The current app serves the workspace and opens the WardensDebt UI in a browser.
...
```powershell
Get-ChildItem WardensDebt\scripts -Recurse -Filter *.js | ForEach-Object { node -c $_.FullName }
```

Manual verification today still means testing WardensDebt behavior in the browser.
...
That will reveal which parts of the original framework are truly reusable and which should be replaced.
