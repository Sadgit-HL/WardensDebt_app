  ...
  loadPromise = (async () => {
    try {
      ...
      const scenario = scenarioId
        ? content.scenarioDefs.find(entry => entry.id === scenarioId)
        : content.scenarioDefs[0];
...
/**
 * Apply a named action with automatic history capture.
 * Replaces manual freezeWdHistory/commitWdHistory calls.
 *
 * Example:
 *   updateWardensDebtGameStateViaAction('move-figure', { figureId: 'c1', cell: { x: 64, y: 96 } })
 */
export function updateWardensDebtGameStateViaAction(actionName, payload) {
  ensureRuntimeReady();
  const draft = structuredClone(runtimeState.gameState);
  const nextGameState = applyAction(draft, actionName, payload);
  return validateAndCommitGameState(nextGameState);
}
